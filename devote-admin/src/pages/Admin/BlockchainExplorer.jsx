...
const BlockchainExplorer = () => {
    ...
    wsRef.current = ApiService.connectWebSocket((data) => {
      if (data.type === "new_transaction") {
        ...
        const notification = {
          ...
          timestamp: new Date(),
          type: getTransactionType(data.data),
        }
  ...
  const getTransactionSummary = (tx) => {
    // Handle both new and legacy transaction formats
    if (tx.data && tx.data.type) {
      // New format with proper data structure
      switch (tx.data.type) {
        case "VOTE":
      ...
      return tx.payload || "System action"
    }
  }
  ...
  const getTransactionType = (tx) => {
    ...
    return tx.payload || "UNKNOWN"
  }

  const getTransactionTimestamp = (tx) => {
    if (tx.data && tx.data.timestamp) {
      return new Date(tx.data.timestamp).toLocaleString()
    }
    return "Unknown time"
  }

  const getTransactionActor = (tx) => {
    if (tx.data && tx.data.actor) {
      // Don't show sensitive user information to regular users
      if (tx.data.actor === "admin") {
        return "System Administrator"
      }
      return tx.data.actor.includes("@") ? "User" : tx.data.actor
    }
    return tx.sender || "Unknown"
  }

  // const getTransactionTarget = (tx) => {
  //   if (tx.data && tx.data.target) {
  //     return tx.data.target
  //   }
  //   return tx.receiver || "Unknown"
  // }

  const getTransactionDetails = (tx) => {
    if (tx.data && tx.data.details) {
      // Filter out sensitive information for regular users
      const filteredDetails = { ...tx.data.details }
      delete filteredDetails.email
      delete filteredDetails.voterID
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-start space-x-4 flex-1">
                        <span className="text-2xl">{getTransactionIcon(getTransactionType(tx))}</span>
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium ${getTransactionTypeColor(getTransactionType(tx))}`}
                            >
                              {getTransactionType(tx).replace("_", " ")}
                            </span>
                            <span className="text-sm text-gray-500">{getTransactionTimestamp(tx)}</span>
                          </div>
          ...
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <span className="text-3xl">{getTransactionIcon(getTransactionType(selectedTransaction))}</span>
                <div>
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-medium ${getTransactionTypeColor(getTransactionType(selectedTransaction))}`}
                  >
                    {getTransactionType(selectedTransaction).replace("_", " ")}
                  </span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{getTransactionTimestamp(selectedTransaction)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700">Actor</label>
                  <p>{getTransactionActor(selectedTransaction)}</p>
                </div>
              </div>

              {Object.keys(getTransactionDetails(selectedTransaction)).length > 0 && (
                <div>
                  <label className="block text-sm font-medium text-gray-700">Additional Information</label>
                  <div className="bg-gray-100 p-3 rounded text-sm">
                    {Object.entries(getTransactionDetails(selectedTransaction)).map(([key, value]) => (
